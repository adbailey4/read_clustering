...
class VariantCallTests(unittest.TestCase):
    ...
    def test_get_variant_set_data(self):
        ...
        assert_frame_equal(temp_df.reset_index(drop=True), (self.vc.get_variant_set_data("Tdm")).reset_index(drop=True), check_dtype = False)
    ...
    def test_get_variant_sets_data(self):
        temp_df = pd.DataFrame(
            {'read_id': ['02636c05-538f-4647-b3ec-8e3a8c5eb10e', '01196b69-900b-4dc0-95a7-169cd79fae9b',
                         '03e6757b-31de-4b13-ab18-57f375404f28', '04c78365-fd9f-4391-8ddb-277620028285',
                         '02d4da5c-ec95-43ac-ac61-9f98ef4a4ca1', '028a34d4-2a7a-44e7-ab23-305915996ec8',
                         ...
                         '02381d7b-ad58-4d21-8ee3-f77401c13814', '02d2f886-87ff-4ab4-98f1-3eeb642f00c2'],
             'contig': ['RDN25-1', 'RDN25-1', 'RDN25-1', 'RDN25-1', 'RDN25-1', 'RDN18-1', 'RDN18-1', 'RDN18-1',
                        'RDN18-1', 'RDN18-1', 'RDN18-1', 'RDN18-1'],
             'reference_index': ['2346', '2346', '2346', '2346', '2346', '1574', '1574', '1574', '1574', '1574', '1574',
                                 '1574'],
             'strand': ['+', '+', '+', '+', '+', '+', '+', '+', '+', '+', '+', '+'],
             'variants': ['Tdm', 'Tdm', 'Tdm', 'Tdm', 'Tdm', 'Gk', 'Gk', 'Gk', 'Gk', 'Gk', 'Gk', 'Gk'],
             'prob1': ['0.902366', '0.722098', '0.739977', '0.794725', '0.840949', '0.256589', '0.970518', '0.947118',
                       '0.301744', '0.496635', '0.215732', '1.000000'],
             'prob2': ['0.048817', '0.138951', '0.130012', '0.102638', '0.079525', '0.743411', '0.029482', '0.052882',
                       '0.698256', '0.503365', '0.784268', '0.000000'],
             'prob3': ['0.048817','0.138951', '0.130012', '0.102638', '0.079525', 'NaN', 'NaN', 'NaN', 'NaN', 'NaN',
                       'NaN', 'NaN']})

        temp_df = temp_df.astype({"reference_index": int, "prob1": float, "prob2": float, "prob3": float})
        assert_frame_equal(temp_df.reset_index(drop=True), (self.vc.get_variant_sets_data(['Tdm', 'Gk'])).reset_index(drop=True),
                           check_dtype = False)

    def test_get_positions_of_variant_sets(self):
        temp_df = pd.DataFrame(
            {'contig': ['RDN25-1', 'RDN18-1'],
             'reference_index': ['2346', '1574'],
             'strand': ['+', '+'],
             'variants': ['Tdm','Gk']})
